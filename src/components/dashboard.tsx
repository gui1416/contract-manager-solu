...
export function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      setContracts(recentContractsData || [])

      const { data: allContractsData, error: allContractsError } = await supabase
        .from("contracts")
        .select("status, end_date, contract_value, created_at");

      if (allContractsError) throw allContractsError;

      const allContracts = allContractsData || [];
      const now = new Date()
      ...
      let totalContractsTrend = "sem alteração";
      if (totalContractsLastMonth > 0) {
        const percentageChange = ((totalContractsThisMonth - totalContractsLastMonth) / totalContractsLastMonth) * 100;
        if (percentageChange !== 0) {
          totalContractsTrend = `${percentageChange > 0 ? '+' : ''}${percentageChange.toFixed(0)}% este mês`;
        }
      } else if (totalContractsThisMonth > 0) {
        totalContractsTrend = `+${totalContractsThisMonth} este mês`;
      }

      const activeContractsThisMonth = allContracts.filter(c => c.status === 'active' && new Date(c.created_at) >= startOfThisMonth).length;
      const activeContractsLastMonth = allContracts.filter(c => c.status === 'active' && new Date(c.created_at) >= startOfLastMonth && new Date(c.created_at) < startOfThisMonth).length;
      const activeContractsDiff = activeContractsThisMonth - activeContractsLastMonth;

      let activeContractsTrend = "sem alteração";
      if (activeContractsDiff !== 0) {
        activeContractsTrend = `${activeContractsDiff > 0 ? '+' : ''}${activeContractsDiff} este mês`;
      }
  ...
  const statsCards = [
    {
      ...
      icon: FileText,
      trend: stats.totalContractsTrend,
    },
    {
    ...
    },
    {
      ...
      icon: CheckCircle,
      trend: stats.activeContractsTrend,
    },
    {
